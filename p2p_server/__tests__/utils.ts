...
import { generateUniqueCode, parseMessage, serializeMessage } from '../src/utils.ts'
...
Deno.test('Message Serialization', async (t) => {
  ...
  await t.step('should serialize messages with numbers', () => {
    ...
    assertEquals(serializeMessage(message as unknown as ActionMessage), 'action:score,points:100')
  })
  ...
  await t.step('should serialize messages with special characters', () => {
    ...
    assertEquals(serializeMessage(message as unknown as ActionMessage), 'action:chat,message:Hello World!')
  })
  ...
  await t.step('should handle empty values', () => {
    ...
    assertEquals(serializeMessage(message as unknown as ActionMessage), 'action:update,data:')
  })
})
