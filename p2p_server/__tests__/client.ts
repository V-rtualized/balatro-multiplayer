...
Deno.test('Client - Static Methods', async (t) => {
  ...
  await t.step('should get client from socket', () => {
    const socket = getMockSocket()
    const client = new Client(socket)
    const retrievedClient = Client.getClientFromSocket(socket)
    assertEquals(retrievedClient, client)
  })

  await t.step('should return undefined for invalid socket', () => {
    const invalidSocket = getMockSocket()
    const retrievedClient = Client.getClientFromSocket(invalidSocket)
    assertEquals(retrievedClient, undefined)
  })
})
...
Deno.test('Client - Cleanup', async (t) => {
  await t.step('should clean up client references', () => {
    ...
    assertEquals(Client.getClientFromSocket(socket), undefined)
  })
