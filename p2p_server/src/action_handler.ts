...
import { ConnectMessage, JoinLobbyMessage, sendType, ToMessage } from './types.ts'

const ActionHandler = {
	connect: async (client: Client, message: ConnectMessage) => {
		if (client.isConnected()) {
			await client.send('action:error,message:Already connected', sendType.Error, "SERVER")
			return
		}

		if (typeof message.username !== 'string') {
			await client.send('action:error,message:Missing username', sendType.Error, "SERVER")
			return
		}

		const username = message.username.trim().substring(0, 20)
		if (username.length === 0) {
			await client.send('action:error,message:Invalid username', sendType.Error, "SERVER")
			return
		}

		client.setConnected(username)
		await client.send(`action:connect_ack,code:${client.getCode()}`, sendType.Ack, "SERVER")
	},

	openLobby: async (client: Client) => {
		if (!client.isConnected()) {
			await client.send('action:error,message:Not connected', sendType.Error, "SERVER")
			return
		}
	...
	joinLobby: async (client: Client, message: JoinLobbyMessage) => {
		...
		if (typeof lobby !== 'string') {
			await client.send('action:error,message:Missing lobby code', sendType.Error, "SERVER")
			return
		}

		const targetLobby = Lobby.getLobby(lobby)
		if (!targetLobby) {
			await client.send('action:error,message:Lobby not found', sendType.Error, "SERVER")
			return
		}
		...
		await host.send(`action:joinLobby,code:${lobby}`, sendType.Sending, "SERVER")
	...
	leaveLobby: async (client: Client) => {
		...
		if (!currentLobby) {
			await client.send('action:error,message:Not in a lobby', sendType.Error, "SERVER")
			return
		}
